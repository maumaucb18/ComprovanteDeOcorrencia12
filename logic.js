...
// Função para conectar-se à impressora Bluetooth pelo nome
async function conectarImpressoraPorNome(nomeImpressora) {
  try {
    // Solicitar permissão para acessar dispositivos Bluetooth
    const device = await navigator.bluetooth.requestDevice({
      filters: [{ name: nomeImpressora }],
...
async function imprimirViaBluetooth() {
  try {
    const nomeImpressora = "Nome da Sua Impressora Bluetooth"; // Substitua pelo nome real da sua impressora
    ...
    const characteristic = await conectarImpressoraPorNome(nomeImpressora);
    if (!characteristic) {
